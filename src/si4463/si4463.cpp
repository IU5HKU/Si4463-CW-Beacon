...
//------------------------------------------------------------------------------------------
CSi4463::CSi4463(uint8_t nSEL, uint8_t mosi, uint8_t miso, uint8_t sck, uint8_t ook, uint8_t sdn)
    : _nSEL(nSEL)
    , _mosi(mosi)
    , _miso(miso)
    , _sck(sck)
...
//------------------------------------------------------------------------------------------
void CSi4463::enterSDN()
{
    digitalWrite(_sdn, HIGH);	//put si4463 into shutdown mode
    MYDEBUG("Enter Shutdown Mode");
}

//------------------------------------------------------------------------------------------
void CSi4463::exitSDN()
{
	digitalWrite(_sdn, LOW);	//wakeup si4463 from shutdown mode
	delay(50);					//wait enough time to give capacitors time to discharge
	MYDEBUG("Exit from Shutdown mode");
}
